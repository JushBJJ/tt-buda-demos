...
from utils.configuration_RW import Qwen2Config
from utils.pybudify import PyBudify
from utils.tt_modeling_RW_pad_masked_odkv import RWForCausalLM as RWForCausalLMPaddedMaskedODKV

MODEL_CKPT = "Qwen/Qwen1.5-0.5B-Chat-GPTQ-Int8"
...
class Qwen:
    ...
    def initialize(self):
        ...
        config = Qwen2Config(**params, user_rows=self.user_rows)
        ...
        # '[PAD]' - set pad token for tokenizer
        self.tokenizer.pad_token_id = 0

        if torch.cuda.is_available():
            self.model.to("cpu")  # change to cuda
            self.device = "cpu"
        else:
            self.device = "cpu"
        ...
        netlist_name = "Qwen2_1024h_16ah_24l_2816i_int8"

        # TODO RE-ADD THIS BEFORE PR
        """
        self.model = PyBudify(
            self.model,
            device="pytorch",
    ...
    def inference(self, prompts):
        ...
        tokenized_attention_mask = tokenized.attention_mask.unsqueeze(
            0).clone()
        ...
        assert min(
            prompt_token_counts) > 0, "Empty prompts for unconditional generation not currently supported"
        ...
        past_key_values = tuple(
            [
                (
                    torch.zeros(
                        (self.batch_size, 1, self.max_length, 64 * 32)),
                    torch.zeros(
                        (self.batch_size, 1, self.max_length, 64 * 32)),
                )
        ...
        while True:
            ...
            for i in range(self.user_rows):
                ...
                if num_tokens < prompt_token_counts[i]:
                    ...
                    attention_mask[:, i,
                                   num_tokens] = tokenized_attention_mask[:, i, num_tokens]
            ...
            position_ids = position_ids.expand(
                self.batch_size, self.user_rows).clone()
            ...
            logits = outputs[0].to("cuda")  # TODO: Change to CPU before PR
